...
import { exercisesMapV2 } from '../utils/exercisesMap';
...
export const filterExercises = async (chosenFilters: IFilter[]) => {
    try {
        if (process.env.REACT_APP_MODE === 'prod') {
            if (process.env.REACT_APP_API_VERSION ==='2'){
                const exercises = await getFilteredExercisesV2(chosenFilters);
                console.log(exercisesMapV2.toExercisesV2Array(exercises));
                return exercisesMapV2.toExercisesV2Array(exercises);
            }
