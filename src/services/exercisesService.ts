import { IExercise, IFilter, IFilterParams, IRatedExercise } from '../types/types';
import * as exercisesService from './exercisesService';
import { getFilteredExercises, getFilteredExercisesV2 } from './exersicesServiceAPI';
...
const filterExercisesLocal = async (chosenFilters: IFilter[], params: IFilterParams) => {
    const exercises = params.rated ?await exercisesService.getRatedExercises() : await exercisesService.getExercises();
    ...
    try {
        if (params.rated) {
            if (process.env.REACT_APP_MODE === 'prod') {
                return await getFilteredExercisesV2(chosenFilters);
            }
            const ratedExercises =  await filterExercisesLocal(chosenFilters, params);
            return sortExercisesByRating(ratedExercises as IRatedExercise[]);
        }
        else {
            if (process.env.REACT_APP_MODE === 'prod') {
                return await getFilteredExercises(chosenFilters);
            }
            return await filterExercisesLocal(chosenFilters, params);
        }
    } catch (err){
        console.error(err);
    }
